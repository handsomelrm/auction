...
class Alreadybuy extends Controller
{
    //买家的已买商品列表
    function alreadyBuyLists()
    {
        //页数
        $page = input('page');
        ...
        //总数
        $totalCount =  Db::name('already_buy')->where('u_id', $u_id)->count('g_id');

        $goods_list = Db::name('already_buy')->alias('ab')
            ->join('goods g', 'ab.g_id=g.g_id')
            ->join('user u', 'ab.u_id=u.u_id')
            ->where('ab.u_id', $u_id)
            ->order('ab.g_id desc')
